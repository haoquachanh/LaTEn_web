 ...
 * This configuration is used for the live application.
 */

import { getConfigValue, getFeatureFlag, type EnvironmentConfig } from './index';
const productionConfig: EnvironmentConfig = {
  api: {
    baseUrl: getConfigValue<string>('SERVER_URL', 'NEXT_PUBLIC_SERVER_URL', 'https://api.laten.edu/api'),
  ...
  features: {
    ...
    enablePerformanceMonitoring: getFeatureFlag('ENABLE_PERFORMANCE_MONITORING', true),
  },

  analytics: {
    enabled: getConfigValue<boolean>('ANALYTICS.ENABLED', 'NEXT_PUBLIC_ANALYTICS_ENABLED', true),
    trackingId: getConfigValue<string>('ANALYTICS.ID', 'NEXT_PUBLIC_ANALYTICS_ID', 'PRODUCTION-TRACKING-ID'),
    options: {
      anonymizeIp: true,
  ...
  debug: {
    enabled: getConfigValue<boolean>('DEBUG_ENABLED', 'NEXT_PUBLIC_DEBUG_ENABLED', false),
    logLevel: getConfigValue<'error' | 'warn' | 'info' | 'debug' | 'trace'>(
      'DEBUG_LOG_LEVEL',
      'NEXT_PUBLIC_DEBUG_LOG_LEVEL',
      'error',
    ),
    showApiErrors: getConfigValue<boolean>('DEBUG_SHOW_API_ERRORS', 'NEXT_PUBLIC_DEBUG_SHOW_API_ERRORS', false),
  },
